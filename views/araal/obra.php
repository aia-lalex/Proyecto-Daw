...
<head>
    ...
    $cliente = new SoapClient(
        null,
        array(
            'location' => "http://localhost/araal/server.php",
            'uri'      => "http://localhost/araal",
            "trace" => 1
        )
    );
    ...
    <div class="container">
        <div class="row mt-5">
            <div class="col">
                ...
                <h2 class="text-center">Insertar materiales en obra</h2>
            </div>
        </div>
        <div class="row">
            <div class="col-12 text-center">
                <div class="panel">
                    <form action="index.php?controller=araal&action=addMateriales" method=post enctype="multipart/form-data"> <!--Formulario-->
                        <label>Nombre:</label><br />
                        <select name="nombre" required>
                            <option value="" disabled>Seleccione</option>
                            <?php
                            $datos = $cliente->listadoAlmacen();
                            if (!empty($datos)) {
                                foreach ($datos as $dato) : // los mostramos los datos
                                    if ($dato['nombre'] == $dF['valores']['nombre']) {
                                        echo '<option value="' . $dato['nombre'] . '" selected>' . $dato['nombre'] . '</option>';
                                    } else {
                        ...
                        <input type="text" name="fecha" value="<?php echo $dF['valores']['fecha']; ?>" /><br />
                        <?php
                        ...
                        <select name="obra" required>
                            ...
                            $dato = $cliente->listadoObra();
                            if (!empty($dato)) {
                                foreach ($dato as $dato) : // los mostramos los datos
                                    if ($dato['nombre'] == $dF['valores']['obra']) {
                                        echo '<option value="' . $dato['nombre'] . '" selected>' . $dato['nombre'] . '</option>';
                                    } else {
                                        echo '<option value="' . $dato['nombre'] . '">' . $dato['nombre'] . '</option>';
                                    }
