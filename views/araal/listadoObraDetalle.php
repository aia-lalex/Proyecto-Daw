...
<body>
    ...
    <div class="container mt-5">
        <div class="row">
            <div class="col">
                <h1 class="text-center">Detalle obra: <?php
                                                        foreach ($datos['datos'] as $dato) {
                                                            echo '' . $dato['nombreObra'] . '';
                                                        } ?> </h1>


                <h2> <?php
                        foreach ($datos['datos'] as $dato) {
                            //EN LA ULTIMA COLUMNA AÑADIMOS LA IMAGEN MONTANDOLA CON LA URL ALMACENADA EN LA BASE DE DATOS
                            $fecha = $dato['fechaoff'];
                            $fechaFormateada = date("d/m/Y", strtotime($fecha));
                            $fechaini = $dato['fechaini'];
                            $fechaFormateada = date("d/m/Y", strtotime($fechaini));
                ...
                <h3 class="text-center">Detalle de materiales: </h3><br>


                <table id="detalle" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                    <thead>
                        <tr>

                            <th>Coste material</th>
                            <th>Fecha</th>
                            <th>Número de materiales usados</th>
                            <th>Material</th>
                            <th>Nombre</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        foreach ($materialesImprimeD['datos'] as $dato) {

                            $fecha = $dato['fecha'];
                            $fechaFormateada = date("d/m/Y", strtotime($fecha));
                        ?>
                            <tr>
                                <td> <?php echo $dato['costeMaterial'] ?> </td>
                ...
                <!-- <h4 class="text-center">Total materiales: <?php /*
                                                            foreach ($materialesImprime['datos'] as $dato) {
                                                                echo '' . $dato['tot'] . '';
                                                            } */ ?> </h4><br>-->

                <br>
                <h4 class="text-center">Detalle horas desde: <?php
                                                                foreach ($datos['datos'] as $dato) {
                                                                    $fecha = $dato['fechaoff'];
                                                                    $fechaFormateada = date("d/m/Y", strtotime($fecha));
                                                                    $fechaini = $dato['fechaini'];
                                                                    $fechaFormateadaIni = date("d/m/Y", strtotime($fechaini));
                                                                    echo '' . $fechaFormateadaIni . '';
                                                                } ?> </h4><br>

                <table id="horas" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
                    <thead>
                        <tr>
                ...
                <h4 class="text-center">Total horas empleadas: <?php
                                                                foreach ($horaSuma['datos'] as $dato) {
                                                                    echo '' . $dato['tot'] . '';
                                                                } ?> </h4>
                <h4 class="text-center">Precio horas empleadas: <?php
                                                                foreach ($horaSuma['datos'] as $dato) {
                                                                    echo '' . $dato['totValor'] . '';
                                                                } ?> </h4><br>
                <br>
                ...
                <h3 class="text-center">Coste total del trabajo: <?php
                                                                    foreach ($datos['datos'] as $dato) {
                                                                        echo '' . $dato['coste'] . '';
                                                                    } ?> Euros</h3>
                ...
                if (!isset($dato) or $dato == null) {
                    echo '<h4>No existe ninguna obra en terminada</h4>';
                }
                ?>
                <a href="index.php?controller=araal&action=listadoObraAdmin">Volver</a>
            </div>
        </div>
    </div>
